...
class PriorityQueue(Heap):
    def __init__(self, V):
        ...
        self.start_idx = 1
        self.A = self.cvt_one_idx(V) # Convert to 1-indexed array
    ...
    def min_heapify(self, i):
        ...
        l = self.left(i)
        r = self.right(i)
        if l <= self.heap_size and self.A[l].d < self.A[i].d: # OOP version: Looks at `d` for key
            smallest = l
        else:
            smallest = i
        if r <= self.heap_size and self.A[r].d < self.A[smallest].d: # If `l` == `r`, then `r` is swapped with `i`
            smallest = r
        if smallest is not i:
            self.swap(i, smallest) # Swap Vertices based on `d` keys - distance estimates
            self.min_heapify(smallest)
        return
    ...
    def extract_root(self, hs_type): # "similar to heap_extract_max"
        ...
        if hs_type == "max":
            ...
            self.min_heapify(1)
        return root # Dijkstra: `root` is a Vertex object containing the distance estimate `d` and the vertex `u`.